...
@bot.message_handler(
    content_types=["sticker", "pinned_message", "photo", "audio", "video"]
)
def warning(message):
...
def help(message):
    bot.send_message(
        message.chat.id,
        f"/start - начать сначала (перезапустить бота)\n/main - главное меню\n/help - вызвать справку",
    )
...
def main(message):
    if message.text == "/main":
        ...
        bot.send_message(
            message.chat.id,
            f"Какой контакт хотите изменить?\nУкажите по:\n/61 - Фамилии\n/62 - Имени\n/63 - Номеру телефона",
        )
        ...
        bot.send_message(
            message.chat.id,
            f"Выберите контакт, который хотите удалить?\nВыберите по:\n/71 - Фамилии\n/72 - Имени\n/73 - Номеру телефона",
        )
...
def change_name(message):
    ...
    lg.logging.info("User entered: {name_it}")
    bot.send_message(message.chat.id, f"{cr.retrive(name=name_it)}")
    bot.send_message(message.chat.id, f"Введите id записи, которую хотите изменить")
...
def change_number(message):
    ...
    bot.send_message(message.chat.id, f"Введите новый номер телефона")
...
def change_new_number(message):
    ...
    bot.send_message(message.chat.id, f"Контакт успешно изменен!")
...
def delete_surname(message):
    ...
    lg.logging.info("User entered: {surname_it}")
    bot.send_message(message.chat.id, f"{cr.retrive(surname=surname_it)}")
    bot.send_message(message.chat.id, f"Введите id записи, которую хотите удалить")
...
def delete_number(message):
    ...
    bot.send_message(message.chat.id, f"Контакт успешно удален!")
